...
const AVATAR_URL = 'https://avatars.githubusercontent.com/agungprsty?s=96';

const HeaderAvatar = ({ isVisible }: { isVisible: boolean }) => (
  <img
    src={AVATAR_URL}
    alt="Agung Prasetyo"
    loading="lazy"
    referrerPolicy="no-referrer"
    className={`rounded-full object-cover transition-all duration-300 origin-left ${
      isVisible ? 'opacity-100 scale-100 w-8 h-8 mr-2' : 'opacity-0 scale-0 w-0 h-0 overflow-hidden'
    }`}
  />
);
        ...
        <h3>
          {homepage ? (
            <a href={homepage} className="link flex items-center">
              <HeaderAvatar isVisible={showNavAvatar} />
              {title}
            </a>
          ) : (
